...
import { analyzeCoupon } from '../api/couponApi'
import type { CouponAnalysisResult } from '../types/coupon'

// Fake example message — for development and testing only, not a real coupon.
...
const EXPIRY_TYPE_LABELS: Record<number, string> = {
...
// Small helper for a labeled field row
      ...
      <form onSubmit={handleAnalyze} className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
        ...
        <button
          ...
          className="w-full bg-blue-600 text-white py-2 rounded-md text-sm font-medium
                     ...
                     transition-colors"
        >
          {mutation.isPending ? 'מנתח...' : 'נתח קופון'}
        </button>
      </form>
      ...
      {result && (
        <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-5">
          ...
          <fieldset className="space-y-3">
            <legend className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">
              זיהוי
            </legend>
            ...
            <FieldRow label="שם מנורמל">
              <input
                type="text"
                value={result.normalizedMerchantName ?? ''}
            ...
            <div className="grid grid-cols-2 gap-3">
              <FieldRow label="סכום מקורי">
                <input
                  ...
                  onChange={(e) =>
                    updateField('originalAmount', e.target.value ? parseFloat(e.target.value) : null)
                  }
                  className={inputClass}
                />
              </FieldRow>
              ...
              <FieldRow label="יתרה">
                <input
                  ...
                  onChange={(e) =>
                    updateField('remainingAmount', e.target.value ? parseFloat(e.target.value) : null)
                  }
                  className={inputClass}
                />
              </FieldRow>
          ...
          <fieldset className="space-y-3">
            ...
            <FieldRow label="תאריך תפוגה">
              <input
                type="date"
                value={toDateInput(result.expirationDate)}
                onChange={(e) =>
            ...
            <FieldRow label="סוג תפוגה">
              <select
                value={result.expirationType}
                onChange={(e) => updateField('expirationType', parseInt(e.target.value))}
                className={inputClass}
              >
                {Object.entries(EXPIRY_TYPE_LABELS).map(([value, label]) => (
                  <option key={value} value={value}>{label}</option>
                ))}
              </select>
            </FieldRow>
          ...
          <button
            type="button"
            disabled
            className="w-full border border-gray-300 text-gray-400 py-2 rounded-md
                       text-sm cursor-not-allowed bg-gray-50"
          >
            שמירה תתווסף בשלב הבא
          </button>
        </div>
      )}
    </div>
